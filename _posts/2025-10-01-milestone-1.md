...
We built **`nhl_pbp`**, a tiny Python package that **pulls NHL play-by-play**, **caches raw JSON**, and **emits tidy per-game and per-season CSVs** so every run is reproducible.  
We also shipped a lightweight interactive debugging tool—**rink plot**, **event scrubber**, and **details pane**—to sanity-check distances and which net a team was attacking.

With a clean dataset, we compared **shot types**, **distances**, and **seasons** to separate what's common from what's effective.  
The picture that emerged is intuitive but quantified: **wrist shots dominate volume**, **close-range deflections and tip-ins over-perform**, and the **6–18 ft band** is the **danger zone**.
