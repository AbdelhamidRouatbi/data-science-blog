...
- Step3 was never defined but we kept the naming of the original instructions to avoid confusion.
- Step 4 shows the final format of our **clean** dataset.
...
- Step 6 calculates the **excess shot rates** for a given season (Figure 6.1) which can be used to improve attack and defense strategies for future matches.
...
**What We Built:**
  - Our interface loads processed game data from our tidy datasets.
...
- Season and Game Selection as dropdown menus to select from available seasons and game types.
...
An interesting part was our assumptions in calculating shot distances. The NHL data gives us the (x, y) coordinates of a shot, but it does not indicate which net the player was shooting at. To solve this, we started by implementing a team-period approach. We looked at where most of a team's shots come from in a single period (negative or positive x coordinates) and assumed that the net they are attacking is located in that zone. After consulting with the teaching team, we were informed that, in the raw JSON data, the home team starts the game by always defending the net located in the negative coordinate zone. Although we corrected our distance calculations accordingly, we did not recreate the figures, as we expect no differences compared to our first approach. Our current code includes both methods and we consider our first approach to be a useful fallback in case we need to work with data having a less clear structure.
...
For this exercice, we chose the season with most recorded events, the 2021-2022 season. The objective is to vizualize in a single figure the relationship between the number of shots, their type (wrist, snap, slap, backhand, tip-in, deflected, wrap-around). Contrary to the instructions, we chose to visualize the success rate of shots (e.g. the rate of goals per shots) instead of the number of goals, since testing showed that the significant difference between "wrist" and the other categories obscures the reading of the figure.
...
For this exercice, distances are grouped into 10-ft bins (e.g., 0–10, 10–20) and computed as the Euclidean distance between each shot’s (x,y) location and each team's net for each period. We chose to present a single figure for all three seasons to compare changes in trends over time.
