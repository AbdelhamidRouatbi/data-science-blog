...
- Step 1 explains how `nhl_pbp` works, a tiny Python package that **pulls NHL play-by-play**, **caches raw JSON**, and **emits tidy per-game and per-season CSVs** so every run is reproducible.
- Step 2 presents a lightweight **interactive debugging tool**—including a rink plot and a details panel— to check the coordinates of events for a certain match.
...
One tricky part was calculating precise shot distances. The NHL data gives us the (x, y) coordinates of a shot, but it doesn't indicate which net the player was shooting at. To solve this, we first implemented a team-period approach. We look at where most of a team's shots come from in a single period and assume that's the net they are attacking. After consulting with the teaching team, we learned that in the raw JSON data, the home team starts the game by always defending the net located in the negative coordinate zone. Although we corrected our distance calculations accordingly, we did not recreate the figures, as we expect no differences compared to our first approach. Our current code includes both methods and we consider our first approach to be a useful fallback in case we need to work with data having a less clear structure.
