...
Model-3’s overall performance is comparable to Model-2B, yet slightly worse overall. However, it uses fewer features, which could be useful for explainability. Thus, we use Model 3 for the final testing in step 6 (see below).

---

# **Question 5: Give it your best shot!**
...
**Regular Season**\
The **XGBoost-3** model that includes only nine features after feature selection in Q4.3, outperformed all other models (CatBoost and LR baselines), with an AUC of approximately **0.875** on the test set, compared to **0.859** on the validation set. The XGBoost (3) model therefore performed better than expected on unseen data. This improvement can likely be explained by the way the data were split as the training and test datasets were separated by season rather than randomly. The 2020/21 regular season data may be simply be easier to predict, for instance due to fewer outliers or lower variability in shots and goals events that took place during that season. The model's overall performance is very similar to that of the CatBoost model, which was trained with slightly different feature engineering and calibration. This indicates that both models capture the underlying relationships in the data, which allows them to make good predictions and seperate high- and low-quality shots.

**Playoffs**\
As in the regular season, **XGBoost-3** again outperformed the other models (CatBoost and LR baselines), although with a slightly lower AUC of **~0.855** compared to validation (0.859). The lower performance on data from playoff (compared to regular-seasons games) likely reflects the greater variability and unpredictability of playoff matches, where conditions are more competitive. In any case, the model was trained on data that included both regular-season and playoff games, and the results here suggest that it generalizes well across both contextes. 
