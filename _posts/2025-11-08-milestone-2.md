...
- Step 5 tests **diverse models** in order to find the best candidates.
- Step 6 presents the final conclusions after **testing** the best models of steps 3-5.

All experiments and the most relevant models are logged on our [Weights & Biases page (WB)](https://wandb.ai/IFT6758_team4/milestone_2).
...
The first step consists of preparing a training set for simple baseline models. To this end, we only use shot distance, shot angle, and whether the net is empty or not. We also perform a brief analysis on this data.
...
- *Figure 4.1. - The four figures for the baseline model (Model-1). The ROC curve shows an AUC of ~0.715 for the validation set. This indicates that the model can distinguish to some degree higher-quality shots from low-quality as it performs better than the 45-degrees line (random classifier).*

The percentile curve (top right) behaves as expected since the actual goal rate increases for shots in bins with higher predicted probabilities. For example, among the 10% of shots the model identified as most dangerous, ~22.5% were actual goals. Similarly, the top 20% of shots by predicted xG account for 40% of all goals, and the top 40% account for 70% of all goals (bottom left). The calibration curve (bottom right) shows that the model does not give high propabilities overal, yet its behavior appears to be correct, along the line of a perfect calibration. See the relevant WB log entry [here](https://wandb.ai/IFT6758_team4/milestone_2/runs/fzldzg63?nw=nwuserantonioslagarias).
...
For both models, we optimize the hyperparameters using a grid search approach with cross-validation (CV).
...
Model 2B’s performance is signficantly better compared to Model 2A. Its overall AUC is higher, among the 10% of shots the model identified as most dangerous, 44% were actual goals (compared to 40% for Model 2A). Similarly, the cumulative curve shows a signficant improvement as the top 20% account for 68% of all goals, while the calibration curve is slightly worse. Therefore, we decided to register model B as an artifact for future use.
...
Our base model uses five features. For each feature added from the candidate set, we check whether the model’s performance improves. Based on these results, we select a total of nine **final features** whose combinatinon seems to give the best result:\
`last_event_distance, time_since_last_event, distance_from_net, empty_net, rebound, period, shot_type, last_event_type and player_count_diff`

We therefore proceed to train model 3 using the above nine features.



**Model-3**\
For model 3, we started from the previous best hyperparameters (from model 2B) and refined the grid. We ran two rounds of cross-validation, adjusting the values, which resulted in the configuration (max_depth=6, eta=0.05, min_child_weight=5, subsample=0.8, colsample_bytree=1.0) with an AUC of 0.86 after 173 rounds, which we kept for training (figure 4.7). See the relevant WB log entry [here](https://wandb.ai/IFT6758_team4/milestone_2/runs/c79urnan?nw=nwuserantonioslagarias).
...
Model-3’s overall performance is comparable to Model-2B, yet slightly worse overall. However, it uses fewer features, which could be useful for explainability. Thus, we use Model-3 in step 6 for the final testing.

#### **Saved XGBoost Models (Artifacts)**
- [xgboost-2A](https://wandb.ai/IFT6758_team4/milestone_2/artifacts/model/xgboost_model2A/v1)
- [xgboost-2B](https://wandb.ai/IFT6758_team4/milestone_2/artifacts/model/xgboost_model2B/v2)
- [xgboost-3](https://wandb.ai/IFT6758_team4/milestone_2/artifacts/model/xgboost_model3/v1)
...
**Protocol.** We froze the models and hyper-parameters from earlier parts and evaluated on the untouched **2020/21** regular-season & playoff-seasons test split. For each figure we plotted six curves: the three LR baselines from Q2, our Q5 **CatBoost (Platt-calibrated)**, the Q4 **best performing XGboost** model (XGboost-3) and a random baseline for reference. We have merged baseline curves along with the best Q4 and Q5 models. We have also provided separate curves for the Q2 baseline models.
...
**Regular Season**\
On the 2020/21 test set, **XGBoost-3** edges **CatBoost (calibrated)** by a very small AUC margin (.8749 vs 0.8719, Δ≈+0.003) and shows higher PR-AUC (≈0.590 vs 0.563). Both boosted models are well ahead of the LR baselines. Both boosted models perform substantially better than the LR baselines. Compared with the validation split (AUC≈0.859), both boosted models improve on the test dataset, performing **better than expected** on unseen data. This improvement is likely due to the way data were split: training and test sets were separated temporally by season rather than randomly. The 2020/21 regular season may have been easier to predict, for example due to fewer outliers or reduced variability in shooting and scoring events. Despite minor differences in feature engineering and calibration, the models capture very similar structure, separating high- and low-quality shots. CatBoost provides well-calibrated probabilities, while XGBoost achieves a marginally higher ranking.

**Playoffs**\
On the 2020/21 playoff set, **XGBoost-3** reaches an AUC of 0.855 (vs 0.8749 of regular season) and **CatBoost (calibrated)** achieves 0.851 (vs 0.8719). The LR baselines follow with lower AUC numbers and the **performance ordering remains unchanged** (XGBoost ≳ CatBoost ≫ LR). This drop in the models' performance likely reflects a higher variance in data from playoff games, where competition is more intense. Since the models were trained on data including both regular-season and playoff matches, the results indicate **good generalization** across both contexts. The ROC, goal-rate, cumulative-goals, and reliability curves show shapes similar to those observed in the regular season, confirming stable performance without any further tuning on the test results.
