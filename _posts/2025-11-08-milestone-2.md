...
**Regular Season**\
On the 2020/21 test set, **XGBoost-3** edges **CatBoost (calibrated)** by a **very small AUC margin** (**0.8749 vs 0.8719**, Δ≈+0.003). Both boosted models are well ahead of the LR baselines and substantially above their earlier baselines. XGBoost shows higher **PR-AUC** (≈0.590 vs 0.563), while CatBoost provides well-calibrated probabilities; taken together, they capture **very similar structure** in the data. This indicates that both models capture the underlying relationships in the data, which allows them to make good predictions and seperate high- and low-quality shots.

**Playoffs**\
All models dip slightly, and the **ordering remains the same** with another **~0.004 AUC** edge for XGBoost. Patterns across ROC/goal-rate/cumulative-goals are consistent with the regular season, indicating **good generalization** from development to both test contexts without any test-time tuning.
